...
const localOptions = { usernameField: 'emailOrUsername' };
...
const localLogin = new LocalStrategy(localOptions, async (emailOrUsername, password, done) => {
  ...
  try {
    ...
    if (!user) user = await User.findOne({ username: emailOrUsername });

    // If no user exists, quit
...
// called with confirmed jwt we just need to confirm that user exits
